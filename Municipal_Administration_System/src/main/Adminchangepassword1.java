...
class Adminchangepassword1 extends JInternalFrame {
	JPanel p1;
	...
	public Adminchangepassword1() {
		setClosable(true);
		setIconifiable(true);
		l1 = new JLabel("HEY Mr/Mrs");
		...
		l4.setBounds(350, 240, 200, 30);
		l4.setForeground(Color.BLUE);
		t4 = new JPasswordField();
		t4.setBounds(600, 240, 160, 30);
		l5 = new JLabel("ADMIN CHANGE PASSWORD");
		l5.setBorder(BorderFactory.createEtchedBorder());
		l5.setBounds(350, 20, 300, 30);
	...
	private void b1actionperformed(ActionEvent e) {
		...
		obj.loaddriver();
		obj.conn();
		obj.fetch("select password from administrator where user_name='" + t1.getText() + "'");
		try {
			obj.rs.next();
			if (obj.rs.getString(1).equals(new String(t2.getPassword()))) {
				if (new String(t3.getPassword()).equals(new String(t4.getPassword()))) {
					String qry = "update administrator set password='" + new String(t3.getPassword())
							+ "' where user_name='" + t1.getText() + "'";
